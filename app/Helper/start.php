...
function sendNotificationToUser($body,$title,$fcm_token,$image)
    {
             $image_url = asset('storage/app/public/notifications') . '/' . $image;
            $SERVER_API_KEY = 'AAAAhqqmtQo:APA91bFnlL3SpOYPnO_FiAmn_K6pBETYGO6UZWT6CHZ8G0aRbe3XJDRZY1uMytanzDDAFD0-ujVR0_y8fMt4Mfpi9PlvoVLVDBJ9MBpyH8OTkEm2LZlQ5nXgBRQ7rCVPuZsysO2B4tgM';
            $data = [
...
function api_encrypt_data_auth($data)
{
        $key = env('AES_KEY');
        $iv = env('AES_IV_KEY');
        $serializedData = json_encode($data,true);
        $encrypted = openssl_encrypt($serializedData,'AES-256-CBC',$key,0,$iv);
        return $encrypted;
   
}
function api_decrypt_data_auth($data)
{
    $key = env('AES_KEY');
    $iv = env('AES_IV_KEY');
...
{
        $aes_key=Company::where('id',user()->company_id)->select('AES_KEY')->first();
       $key = $aes_key->AES_KEY;
        $iv = env('AES_IV_KEY');
        $serializedData = json_encode($data,true);
        $encrypted = openssl_encrypt($serializedData,'AES-256-CBC',$key,0,$iv);
