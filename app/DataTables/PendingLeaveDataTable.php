...
class PendingLeaveDataTable extends BaseDataTable
{
        ...
        return datatables()
            ...
            ->addColumn('duration', function ($row) {
                $leave = ' <div class="media-body">
                    <span class="mb-0 f-13">' . (
                    ($row->start_time == 1 || $row->end_time == 1)  && ($row->duration !== 'half_day')
                        ? "Short Leave"
                        : (
                            ($row->duration == 'half_day')
                                ? __('modules.leaves.halfDay')
                                : (
                                    ($row->duration == 'multiple')
                                        ? __('modules.leaves.multiple')
                                        : __('app.'.$row->duration)
                                )
                        )
                ) . '</span></br>';
            ...
            ->addColumn('leave_type', function ($row) {
                $type = '<span class="badge badge-success" style="background-color:' . $row->color . '">' . $row->type_name . '</span></br>';

                if ($row->duration == 'half_day') {
                 ...
                 if ($row->type_name == 'Short Leave') {
                        $type .= ' <div class="badge-inverse badge">' . (($row->start_time) ? __('modules.leaves.firstHalf') : __('modules.leaves.secondHalf')) . '</div>';
                }
            ...
            ->addColumn('action', function ($row) {
                ...
                if ($row->status == 'pending' && $this->reportingTo && $row->user_id != user()->id && !in_array('admin', user_roles())) {
                    ...
                    if ($row->manager_status_permission == '' && !($this->reportingPermission == 'cannot-approve')  && $row->duration !== 'multiple') {
                    ...
                    if ($this->reportingPermission == 'approved' &&  $row->duration !== 'multiple' && $row->manager_status_permission == '')
                    {
                        $actions .= '<a class="dropdown-item leave-action-approved" data-leave-id=' . $row->id . '
                                 data-leave-action="approved" data-user-id="' . $row->user_id . '" data-leave-type-id="' . $row->leave_type_id . '" href="javascript:;">
                                    <i class="fa fa-check mr-2"></i>
                    ...
                    elseif ($this->reportingPermission == 'pre-approve' && !$row->manager_status_permission  && $row->duration !== 'multiple') {
                        $actions .= '<a data-leave-id=' . $row->id . '
                    ...
                    if (($row->duration === 'multiple' && !is_null($row->unique_id)) && $this->reportingPermission == 'approved') {
                        $actions .= '<a class="dropdown-item view-related-leave" data-leave-id=' . $row->id . '
                                 data-unique-id="' . $row->unique_id . '" data-leave-type-id="' . $row->leave_type_id . '" href="javascript:;">
                                    <i class="fa fa-eye mr-2"></i>
                                    ' . __('app.view') . ' '.__('modules.leaves.relatedLeave').'
    ...
    public function query(Leave $model)
    {
        ...
        ->selectRaw('leaves.*, leave_types.color, leave_types.type_name, 
                     COUNT(leaves.id) as count_multiple_leaves')
        ->groupBy('leaves.unique_id', 'leaves.half_day_type');
        ...
        $leavesList->where('leaves.status','pending');
