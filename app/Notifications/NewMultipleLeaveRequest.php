...
class NewMultipleLeaveRequest extends BaseNotification
{
    ...
    public function toMail($notifiable): MailMessage
    {
        ...
        $emailDate = '';
            $emailDate .= $dates;
    $dateArray = explode(',', $this->multiDates);
...
$formattedLastDate = $lastDateTime->format("d M");
        $content = __('email.leaves.subject') . ' ' . __('app.from') . ' ' . $this->leave->user->name . '.' . '<p><b>' . __('modules.leaves.leaveType') . ':</b> ' . $this->leave->type->type_name . '</p><p><b>' . __('modules.leaves.reason') . '</b></p><p>' . $this->leave->reason . '</p><p><b>' . __('app.leaveDate') . '</b></p><p>From: '.$formattedFirstDate . ' To: ' .$formattedLastDate .'</p>';

        return $build
          ->subject(__('email.leaves.subject') . ' From ' . $this->leave->user->name .' (' .$this->leave->type->type_name.')')
