...
class LeaveStatusApprove extends BaseNotification
{
    ...
    public function toMail($notifiable)
    {
        ...
        $all_leaves = Leave::where('unique_id',$this->leave->unique_id)->get();
        $content = __('email.leave.approve') . '<br>' . __('app.date') . 'From : ' . Carbon::parse($all_leaves->first()['leave_date'])->format('d M') .  ' To : ' . Carbon::parse($all_leaves->last()['leave_date'])->format('d M') . '<br>' . __('app.status') . ': ' . $this->leave->status . '<br>';
