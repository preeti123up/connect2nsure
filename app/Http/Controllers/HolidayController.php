...
class HolidayController extends AccountBaseController
{
    ...
    public function store(CreateRequest $request)
    {
        ...
        foreach ($dates as $index => $value) {
            if ($value != '') {
                ...
                $holiday->holiday_type = $holiday_type[$index];
                if ($request->hasFile('image')) {
                    $holiday->app_banner = Files::uploadLocalOrS3($request->image[$index], 'holidays', 300);
                }
                $holiday->save();
    ...
    public function update(UpdateRequest $request, Holiday $holiday)
    {
        ...
        if ($holiday) {
            $holiday->event_id = $this->googleCalendarEvent($holiday);
            if ($request->hasFile('image')) {
                Files::deleteFile($holiday->app_banner , 'holidays');
                $holiday->app_banner = Files::uploadLocalOrS3($request->image, 'holidays', 300);
            }
            $holiday->save();
        }
