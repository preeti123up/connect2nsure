...
class AttendanceController extends AccountBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            if($request->status=='deactive'){
                $employees = User::where(['company_id'=>user()->company_id,'status'=>$request->status])->withoutGlobalScope(ActiveScope::class)->select('id','name','status')->get();

            }else{
                $employees = User::allEmployees(null, true, ($this->viewAttendancePermission == 'all' ? 'all' : null));
            }
            return response()->json(['user_id'=>$request->userId,'user'=>$employees,'data'=>$this->summaryData($request)]);
        }
        ...
        $this->branch = Branch::where('company_id',user()->company_id)->get();
    ...
    public function summaryData($request)
    {
        ...
        ->join('role_user', 'role_user.user_id', '=', 'users.id')
            ...
            ->select('users.id', 'users.name', 'users.email', 'users.created_at', 'employee_details.department_id', 'users.image','users.ood')
            ->onlyEmployee()
            ->groupBy('users.id');
          ...
          if ($request->status != 'all' && $request->status != '') {
                    ...
                    $employees = $employees->where('users.status', $request->status);
            }
        ...
        if ($request->designation != 'all') {
         ...
         if($request->userId != 'all' && $request->userId == user()->id) {
            $employees = $employees->where('users.id', $request->userId);
            $employees = $employees->orWhere('employee_details.reporting_to', (int)$request->userId);
        }
        ...
        if ($request->branch != 'all' && $request->branch != '') {
            $assign=AssignBranch::where('branch_id',$request->branch)->pluck('user_id')->toArray();
           
            $employees = $employees->whereIn('employee_details.user_id', $assign);
    ...
    // Check for pending location and working from outside
    if ($attendance->status == "pending" && $attendance->working_from == "outside") {
        $text_color = "text-yellow";
    }

    // Initialize the HTML for final output
    $html = '<a href="javascript:;" data-toggle="tooltip" data-original-title="' . ($attendance->employee_shift_id ? $attendance->shift->shift_name : __('app.present')) . '" class="view-attendance ' . $text_color . '" data-attendance-id="' . $attendance->id . '">';
    $html .= '<i class="fa fa-' . ($attendance->half_day == 'yes' ? 'star-half-alt' : ($attendance->late == 'yes' ? 'exclamation-circle' : 'check')) . '"></i>';
    $html .= '</a>';

    // Check if shift schedule is available for the day
    if (isset($shiftScheduleCollection[$clockInTime->copy()->startOfDay()->toDateTimeString()])) {
        ...
        if ($clockInTime->between($shiftStartTime, $shiftEndTime)) {
            $final[$employee->id . '#' . $employee->name][$clockInTime->day] = $html;
            ...
            $final[$employee->id . '#' . $employee->name][$clockInTime->copy()->subDay()->day] = $html;
        } else {
            $final[$employee->id . '#' . $employee->name][$clockInTime->day] = $html;
        }
    } else {
        // If shift schedule is not available
        $final[$employee->id . '#' . $employee->name][$clockInTime->day] = $html;
    }
}
            ...
            foreach ($employee->leaves as $leave) {
                 if ($leave->type->type_name=='Short leave'||$leave->type->type_name=='Short Leave'||$leave->type->type_name=='short_leave'||$leave->type->type_name=='Short_Leave'||$leave->type->type_name=='Short_leave'){
                    if ($final[$employee->id . '#' . $employee->name][$leave->leave_date->day] == '-' || $final[$employee->id . '#' . $employee->name][$leave->leave_date->day] == 'Absent') {
                        // Check the type of leave only for half-day leave
                        if ($leave->type->type_name == 'short_leave') {
                            if ($leave->status == 'approved') {
                                $final[$employee->id . '#' . $employee->name][$leave->leave_date->day] = 'short_leave';
                            }
                        } else {
            ...
            foreach ($this->holidays as $holiday) {
             ...
             if (!($employee->shifts->count())) {
                $attendanceSettings = AttendanceSetting::first()->shift;
                foreach ($final[$employee->id . '#' . $employee->name] as $i => $v) {
                    $dateString = $i . '-' . $request->month . '-' . $request->year;
                    if (strtotime($dateString) !== false) {
                        $dayOfWeek = Carbon::parse($dateString)->dayOfWeek;
                        $officeOpenDays = array_map('intval', $attendanceSettings->office_open_days);
                        if (!in_array($dayOfWeek, $officeOpenDays)) {
                            if($final[$employee->id . '#' . $employee->name][$i] == 'Absent' || $final[$employee->id . '#' . $employee->name][$i] == '-'){
                                $final[$employee->id . '#' . $employee->name][$i] = 'Day Off';
                            }
                         }
                        }
                    }
    ...
    public function employeeData(Request $request, $startDate = null, $endDate = null, $userId = null)
    {
        ...
        // Getting Leaves Data
          $leavesDates = Leave::with(['type'])
        ->where('user_id', $userId)
        ->where('leave_date', '>=', $startDate)
        ->where('leave_date', '<=', $endDate)
        ->whereHas('type', function ($query) {
            $query->where('type_name', '!=', 'Short Leave');
        })
        ->where('status', 'approved')
        ...
        ->keyBy('date') 
        ->toArray();
        $totalWorkingDays = $totalWorkingDays-$week_off-count($holidays);
        ...
        $daysAbsent = (($totalWorkingDays - $daysPresent - count($leavesDates)) < 0) ? '0' : ($totalWorkingDays - $daysPresent -count($leavesDates));
        ...
        for ($date = $endDate; $date->diffInDays($startDate) > 0; $date->subDay()) {
            ...
            if ($date->isPast() || $date->isToday()) {
                ...
                //check week off
                $attendanceSettings = EmployeeShiftSchedule::where('user_id', $userId)
                ...
                if(!in_array($date->copy()->dayOfWeek, $attendanceSettings->office_open_days)){
                    $dateWiseData[$date->toDateString()]['holiday'] = "Week Off";
                }
                //end check Off
    ...
    public function exportAllAttendance($date,$year, $month, $id, $department, $designation,$branch)
    {
        abort_403(!canDataTableExport());
        $data['date'] = $date;
        $data['month'] =$month;
        ...
        $data['branch']=$branch;
        $currentMonth = Carbon::parse('01-' . $month . '-' . $year);
        $currentMonthName = $currentMonth->format('F'); 
        return Excel::download(new AttendanceExport($data), $currentMonthName . '.xlsx');
    }
    ...
    public function approveOrRejectAttendance(Request $request, $id)
    {
        $pendingAttendance = Attendance::where('id', '=', $id)->first();
        if ($pendingAttendance) {
            $pendingAttendance->status = $request->action;
            $pendingAttendance->approved_by = auth()->user()->id;
            $pendingAttendance->approved_at = now();
    ...
    {
        $this->pendingAttendance = Attendance::with('user')->where('id', $id)->first();
        return view('attendances.ajax.list', $this->data);
    }
        ...
        function pendingAttendance(PendingAttendanceDataTable $dataTable){
        $this->pendingAttendance = Attendance::with('user')->where('status','=','pending')->get();
        return $dataTable->render('pending-attendance.index', $this->data);
