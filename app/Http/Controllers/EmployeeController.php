...
use App\Exports\EmployeeMultiSheetExport;
...
class EmployeeController extends AccountBaseController
{
    ...
    public function assignRole(Request $request)
    {
        ...
        $userSession->deleteSessions([$userId]);
    ...
    public function store(StoreRequest $request)
    {
        ...
        try {
            ...
            $branches =  $request->assign_branch;
            if(!empty($branches))
            {
                foreach($branches as $branch){
                    $employeeBranch = new AssignBranch();
                    $employeeBranch->user_id = $user->id;
                    $employeeBranch->branch_id = $branch;
                    $employeeBranch->save();
                }
            }
    ...
    public function applyQuickAction(Request $request)
    {
        ...
        case 'send-email':
             ...
             return Reply::success(__('Send Email Successfully'));
        case 'change-status':
    ...
    public function edit($id)
    {
        ...
        $this->salutations = Salutation::cases();
        $this->branches = Branch::where(['company_id' => company()->id])->get();
        $this->getBranchIds = AssignBranch::where(['user_id'=>$id])->get()->pluck('branch_id')->toArray();
        $this->bankNames = BankNameMasters::get();
    ...
    public function update(UpdateRequest $request, $id)
    {
        ...
        $branches =  $request->assign_branch;
       EmployeeBranch::where(['emp_id'=>$user->id])->delete();
       if(!empty($branches))
        {
            foreach($branches as $branch){
                $employeeBranch = new AssignBranch();
    ...
    public function show($id)
    {
        ...
        if (
            ...
            if ($tab == '') {  // Works for profile
                ...
                $this->leavesTaken = Leave::selectRaw('count(*) as count, SUM(if(duration="half_day", 1, 0)) AS halfday')
                    ->where('user_id', $id)
    ...
    public function exportFormate(){

        return Excel::download(new EmployeeMultiSheetExport(), 'employee_bulk_format.xlsx');
    }
       protected function sendEmailUser($request)
    {
        $users = User::whereIn('id', explode(',', $request->row_ids))->get();
        $users->each(function ($user) {
            try{
                event(new NewUserEvent($user,12345678));
                $user->update(['email_send' => 1]);
            }catch(Exception $e){
                $user->update(['email_send' => 0]);
            }
         });
