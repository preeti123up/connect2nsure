             ...
             if (in_array('leaves', $calendar_filter_array)) {
                ...
                $leaves = Leave::join('leave_types', 'leave_types.id', 'leaves.leave_type_id')
                    ...
                    if(in_array('employee', user_roles()) && !(in_array('admin', user_roles()))){
                        $reporting_users = EmployeeDetails::where('reporting_to', user()->id)->pluck('user_id')->toArray();
                       
                        $leaves = $leaves->where('leaves.user_id', user()->id)
                        ->orWhereIn('leaves.user_id', $reporting_users)->get();
                        
                    }
                   

                foreach ($leaves as $leave) {
                    $duration = ($leave->duration == 'half_day') ? '( ' . __('app.halfday') . ' )' : '';
                  if($leave->type_name == 'Short Leave'){
