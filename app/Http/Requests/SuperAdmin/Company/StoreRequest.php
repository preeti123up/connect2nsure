...
class StoreRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        $rules = [
            ...
            'company_email' => 'required|email|unique:companies|regex:/^\b[A-Z0-9._%-]+@[A-Z0-9.-]+\.[A-Z]{2,4}\b$/i',
            'mobile' => 'required|unique:users|digits:10|regex:/^[6-9][0-9]{9}$/',
            'address' => 'required',
            'sub_domain' => module_enabled('Subdomain') ? 'required|banned_sub_domain|min:' . $len . '|unique:companies,sub_domain|max:50' : '',
            'status' => 'required',
            'email' => 'required|email:rfc|regex:/(.+)@(.+)\.(.+)/i|check_superadmin|unique:users|regex:/^\b[A-Z0-9._%-]+@[A-Z0-9.-]+\.[A-Z]{2,4}\b$/i',
            'name' => 'required|min:2',
