...
class StoreRequest extends CoreRequest
{
    ...
    public function rules()
    {
        ...
        $storedCaptchaCode = session('default_captcha_code');
         Validator::extend('check_captcha_code', function ($attribute, $value, $parameters, $validator) use ($storedCaptchaCode) {
            return $value === $storedCaptchaCode;
        });
        $rules = [
           'company_name' => 'required',
            'company_phone' => 'required|regex:/^[6-9]{1}[0-9]{9}$/',
            'name' => 'required|regex:/^[a-zA-Z\s]+$/',
            'captcha' => 'required|check_captcha_code', 
            'email' => 'required|email|regex:/(.+)@(.+)\.(.+)/i|check_superadmin',
    ...
    public function messages()
    {
        return [
            ...
            'check_captcha_code' => 'invalid Captcha',
        ];
    }
