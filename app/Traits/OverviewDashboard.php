...
trait OverviewDashboard
{
    ...
    public function overviewDashboard()
    {
       ...
       $this->pendingAttendance = Attendance::with('user')->where('status','=','pending')
            ->get();
            
         $this->pendingAssets = Asset::with(['lendedAsset.user'])
        ...
        $this->pendingLeadFollowUps = Lead::with('followup', 'leadAgent', 'leadAgent.user', 'leadAgent.user.employeeDetail', 'leadAgent.user.employeeDetail.designation')
            ->selectRaw(
                'leads.id,leads.company_name, leads.client_name, leads.agent_id, ( select lead_follow_up.next_follow_up_date from lead_follow_up where lead_follow_up.lead_id = leads.id and DATE(lead_follow_up.next_follow_up_date) < "' . $currentDate . '" ORDER BY lead_follow_up.created_at DESC Limit 1) as follow_up_date_past,
