...
class AttendanceByMemberExport implements FromCollection, WithHeadings,  WithMapping
{
    ...
    public function collection()
    {
        ...
        leftJoin('company_addresses', 'company_addresses.id', '=', 'attendances.location_id')
            ...
            $leavesDates = Leave::whereHas('type', function($query) {
        ...
        foreach ($attendances->sortBy('date') as $attendance) {
            ...
            $checkdate = Carbon::createFromFormat('Y-m-d H:s:i', $attendance->date)->timezone(company()->timezone)->startOfDay(); // Ensure we're comparing dates, not datetimes
            ...
            if ($attendance->status != null) {
             ...
             // Check if leave and attendance also overlap
        foreach ($leavesDates as $leave) {
            if ($checkdate->equalTo(Carbon::createFromFormat('Y-m-d H:s:i', $leave->leave_date)->timezone(company()->timezone)->startOfDay())) {
                if($status == "Present"){
                    $status =   $status . '( On Leave -' . $leave->half_day_type .')';
                }
