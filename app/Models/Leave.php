...
class Leave extends BaseModel
{
    ...
    public static function getLeaveCounts($user_id, $leave_id)
    {
        $halfDayCount = self::where('user_id', $user_id)
            ->where('leave_type_id', $leave_id)
            ->where('duration', 'half_day')
            ->whereIn('status', ['pending', 'approved'])
            ->count();
    
        $fullDayCount = self::where('user_id', $user_id)
            ->where('leave_type_id', $leave_id)
            ->where('duration', '!=', 'half_day')
            ->whereIn('status', ['pending', 'approved'])
            ->count();
    
        return ($halfDayCount / 2) + $fullDayCount;
    }
