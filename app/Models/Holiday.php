...
class Holiday extends BaseModel
{
    ...
    protected $fillable = ['date', 'occassion','holiday_type','app_banner'];
    ...
    {
        return ($this->app_banner) ? asset_url_local_s3('holidays/' . $this->app_banner, true, 'image') : null;
    }
 ...
 public static function getGazettedHolidayByDates($startDate, $endDate)
    {
        return Holiday::select(DB::raw('DATE_FORMAT(date, "%Y-%m-%d") as holiday_date'), 'occassion')
            ->where('date', '>=', $startDate)
            ->where('date', '<=', $endDate)
