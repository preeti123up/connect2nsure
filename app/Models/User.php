...
class User extends BaseModel
{
    ...
    public static function allEmployees($exceptId = null, $active = false, $overRidePermission = null, $companyId = null)
    {
        ...
        $users = User::withRole('employee')
            ...
            ->select('users.id', 'users.company_id', 'users.name', 'users.created_at', 'users.image', 'designations.name as designation_name', 'users.email_notifications', 'users.mobile', 'users.email', 'users.country_id');
        ...
        if (!isRunningInConsoleOrSeeding() && user()) {
            if (isset($viewEmployeePermission)) {
                ...
                elseif ($viewEmployeePermission == 'both' && !in_array('client', user_roles())) {
                    $users->where(function ($q) {
                        $q->where('employee_details.user_id', user()->id);
                     $q->orWhere('employee_details.reporting_to', user()->id);
                    });
            ...
            foreach ($this->cachedRoles() as $role) {
                ...
                foreach ($role->cachedPermissions() as $perm) {
                    if($perm->name == $permission){
                        $currentpermission = PermissionRole::with('permissionType')->where('permission_id', $perm->id)->first();
                        if($currentpermission->permissionType->name == "none"){
                            return false;
                        }else{
                            return true;
                        }
    ...
    public function assignUserRolePermission($roleId)
    {
        ...
        foreach ($rolePermissions as $permission) {
              $cache_permission = Permission::find($permission->permission_id);
        Cache::forget('permission-' . $cache_permission->name . '-' . $this->id);
            $data[] = [
    ...
    public function userBadge()
    {
        ...
        if (user() && user()->id == $this->id) {
           ...
           if($this->ood == "enable"){
            return $salutation . $this->name . ' <span class="ml-2 badge  badge-danger pr-1">Outside</span>';
        }
