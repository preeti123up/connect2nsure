...
class LeaveObserver
{
    ...
    { 
        if (!isRunningInConsoleOrSeeding()) {
            if (request()->duration == 'multiple') {
                $dateArray = explode(',', request()->multi_date);
                $countLeavesForDate = Leave::whereIn('leave_date', $dateArray)->count();
                if ($countLeavesForDate == 1) {
            ...
            if (!is_null($leave->leave_date) && 
                !is_null($leave->user) && 
                !is_null($leave->user->calendar_module) && 
                $leave->user->calendar_module->leave_status) {
    ...
    public function updated(Leave $leave)
    {
        if (!isRunningInConsoleOrSeeding()) {
            ...
            if ($leave->isDirty('status')) {
                $ifMultiple = Leave::where('unique_id',$leave->unique_id)->first();
                if($ifMultiple->id === $leave->id){
