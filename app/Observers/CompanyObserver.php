...
//        $company->light_logo = $globalSetting->light_logo;
    ...
    public function employeeShift($company)
    {
        ...
        $employeeShift->office_open_days = ["6","7"];
    ...
    public function leaveType($company)
    {
        ...
        $status = [
            ...
            ['type_name' => 'LWP', 'color' => '#B078C6', 'company_id' => $company->id, 'gender' => json_encode($gender), 'marital_status' => json_encode($maritalstatus),  'role' => json_encode($roles)]
        ];
    ...
    public function updateModuleSettings($company): void
    {
        ...
        foreach ($moduleInPackage as $key => $value) {
            $ifExists = ModuleSetting::where('module_name', $value)
            ->where('company_id',$company->id)
            ->exists();
            if ($ifExists) {
                $moduleInPackage->forget($key);
            }
        }
        
    if($moduleInPackage->count()){
        $data = [
            'admin' => $moduleInPackage->toArray(),
            'employee' => $moduleInPackage->toArray(),
        ]; 
        $moduleSettings = [];
