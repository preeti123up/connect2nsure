...
<div class="modal-header">
    <h5 class="modal-title">@lang('Add Bank Name')</h5>
        ...
        $.easyAjax({
            ...
            success: function (response) {
                if (response.status === 'success') {
                    $('#myModal').modal('hide');
                        $('#bank_name').append('<option value="' + response.data.name + '">' + response.data.name + '</option>');
    ...
    $('body').on('click', '.delete-field', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        let url = "{{ route('bank-name.destroy',':id') }}";
