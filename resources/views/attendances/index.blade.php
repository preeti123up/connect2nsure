...
@section('filter-section')
    ...
    <x-filters.filter-box>
            ...
            <div class="select-status">
                <select class="form-control select-picker" name="user_id" id="user_id" data-live-search="true"
                    ...
                    @if (in_array('admin', user_roles()))
                        ...
                        <x-user-option :user="$item" ></x-user-option>
                    @empty
                        <x-user-option :user="user()"></x-user-option>
                    @endforelse
                    @else
                     @forelse ($employees as $item)
                        <x-user-option :user="$item" :selected="user()->id == $item->id"></x-user-option>
                    @empty
                        <x-user-option :user="user()"></x-user-option>
                    @endforelse
                    @endif
        ...
        <div class="select-box d-flex py-2 px-lg-2 px-md-2 px-0 border-right-grey border-right-grey-sm-0">
            <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">@lang('app.select') @lang('app.date')</p>
            <div class="select-status">
                  <input class="form-control" data-date=""  name="att_date" type="date" id="att_date" value="">
            </div>
        </div>
...
@push('scripts')
    ...
    <script>
        ...
        $('#user_id, #department, #designation, #month, #year,#branch,#status').on('change', function () {
            if ($('#user_id').val() != "all") {
        ...
        @if (canDataTableExport())
             $('#export-all').click(function () {
               ...
               var status=$('#status').val();
                ...
                var url =
                    "{{ route('attendances.export_all_attendance', [':date',':year', ':month', ':userId', ':department', ':designation',':branch']) }}";
                url = url.replace(':date', date).replace(':year', year).replace(':month', month).replace(':userId', userId).replace(':department', department).replace(':designation', designation).replace(':branch',branch).replace(':status',status);
