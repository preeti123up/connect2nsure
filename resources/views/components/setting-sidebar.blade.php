...
<div class="settings-sidebar bg-white py-3" id="mob-settings-sidebar">
    ...
    <ul class="settings-menu" id="settingsMenu">
        ...
        @if(in_array('admin', user_roles()))
            <x-setting-menu-item  :active="$activeMenu" menu="branch" :href="route('branch.index')" :text="__('Branch')" />
        @endif
         @if(in_array('admin', user_roles()))
                <x-setting-menu-item  :active="$activeMenu" menu="Branch Assign" :href="route('assign-branch.index')" :text="__('Branch Assign')" />
         @endif
          @if (in_array('admin', user_roles()))
            <x-setting-menu-item :active="$activeMenu" menu="wishes_setting" :href="route('wishes.index')" :text="__('app.menu.wishes_setting')" />
        @endif
