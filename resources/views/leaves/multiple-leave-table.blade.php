<x-table class="table-sm-responsive table mb-0">
    <x-slot name="thead">
        ...
        @if ($approveRejectPermission === 'all' || ($deleteLeavePermission == 'all'
                                || ($deleteLeavePermission == 'added' && user()->id == $multipleLeaves->first()->added_by)
                                || ($deleteLeavePermission == 'owned' && user()->id == $multipleLeaves->first()->user_id)
                                || ($deleteLeavePermission == 'both' && (user()->id == $multipleLeaves->first()->user_id || user()->id == $leave->added_by))))
