...
<!-- NAV ITEM - CUSTOMERS COLLAPASE MENU -->
    @if (!in_array('client', user_roles()) && in_array('leads', user_modules())  && user()->permission('view_lead') != 'none')
...
@if (!in_array('client', user_roles()) && (in_array('employees', user_modules())|| in_array('leaves', user_modules())|| in_array('attendance', user_modules()) || in_array('holidays', user_modules())) && (user()->permission('view_employees') != 'none'|| user()->permission('view_leave') != 'none'|| user()->permission('view_attendance') != 'none'|| user()->permission('view_holiday') != 'none'))
        <x-menu-item icon="people" :text="__('app.menu.hr')">
            ...
            <div class="accordionItemContent pb-2">
                @if (in_array('employees', user_modules())  && user()->permission('view_employees') != 'none')
                    <x-sub-menu-item :link="route('employees.index')" :text="__('app.menu.employees')" />
                     <x-sub-menu-item :link="route('leaves.pending')" :text="__('Pending Leaves')" />
                @endif
                @if (in_array('leaves', user_modules()) && user()->permission('view_leave') != 'none')
                    <x-sub-menu-item :link="route('leaves.index')" :text="__('app.menu.leaves')" />
                @endif
                @if (in_array('attendance', user_modules())  && user()->permission('view_shift_roster') != 'none')
                    <x-sub-menu-item :link="route('shifts.index')" :text="__('app.menu.shiftRoster')" />
                @endif
                @if (in_array('attendance', user_modules()) && user()->permission('view_attendance') != 'none')
                    <x-sub-menu-item :link="route('attendances.index')" :text="__('app.menu.attendance')" />
                @endif
                @if (in_array('holidays', user_modules()) && user()->permission('view_holiday') != 'none')
                ...
                @endif
                @if (isset($sidebarUserPermissions['view_appreciation']) && user()->permission('view_appreciation') != 'none')
                ...
                @endif
                @if (in_array('admin', user_roles()) )
                    <x-sub-menu-item :link="route('pendingAttendance.index')" :text="__('Pending Attendance')" />
                @endif
            </div>
        </x-menu-item>
    @endif
...
@if((in_array('contracts', user_modules()) || in_array('projects', user_modules()) || in_array('tasks', user_modules()) || in_array('timelogs', user_modules())) && ( user()->permission('view_contract') != 'none' || user()->permission('view_projects') != 'none' || user()->permission('view_tasks') != 'none' || user()->permission('view_timelogs') != 'none'))
        <x-menu-item icon="briefcase" :text="__('app.menu.work')">
            ...
            <div class="accordionItemContent pb-2">
                ...
                @if (in_array('tasks', user_modules()) && user()->permission('view_tasks') != 'none')
                    <x-sub-menu-item :link="route('tasks.index')" :text="__('app.menu.tasks')" />
                @endif
                @if (in_array('timelogs', user_modules()) && user()->permission('view_timelogs') != 'none')
...
<!-- NAV ITEM - FINANCE COLLAPASE MENU -->
 @if ((in_array('estimates', user_modules()) || in_array('invoices', user_modules()) || in_array('payments', user_modules()) || in_array('expenses', user_modules()) || in_array('bankaccount', user_modules())) &&  
 (user()->permission('view_estimates') != 'none'|| user()->permission('view_invoices') != 'none'  || user()->permission('view_payments') != 'none' || user()->permission('view_expenses') != 'none'|| user()->permission('view_lead_proposals') != 'none'|| user()->permission('view_bankaccount') != 'none'))
        <x-menu-item icon="cash-coin" :active="($currentRouteName === 'payments.index')"
            ...
            <div class="accordionItemContent pb-2">
                ...
                @endif
                @if (in_array('estimates', user_modules()) && user()->permission('view_estimates') != 'none')
                    <x-sub-menu-item :link="route('estimates.index')" :text="__('app.menu.estimates')" />
                @endif
                @if (in_array('invoices', user_modules()) && user()->permission('view_invoices') != 'none')
                    <x-sub-menu-item :link="route('invoices.index')" :text="__('app.menu.invoices')" />
                @endif
                @if (in_array('payments', user_modules()) && user()->permission('view_payments') != 'none')
                ...
                @if (in_array('expenses', user_modules()) && user()->permission('view_expenses') != 'none')
...
<!-- NAV ITEM - PRODUCTS -->
@if (!module_enabled("Purchase") && in_array('products', user_modules()) && user()->permission('view_product') != 'none')
...
<!-- Knowledge base -->
    @if (in_array('knowledgebase', user_modules()) && user()->permission('view_knowledgebase') != 'none')
...
<!-- Knowledge base -->
    ...
    <!-- NAV ITEM - NOTES -->
    @if (in_array('client', user_roles()) && user()->permission('view_client_note') != 'none')
...
@endif
@php
$assign=App\Models\AssignInterview::where('assign_to',user()->id)->count();
@endphp

@if((in_array('interview', user_modules())  && $assign) || (in_array('interview', user_modules()) &&in_array('admin', user_roles())))
<x-menu-item icon="question" :text="__('Interview Application')" :link="route('interview.index')">
                <x-slot name="iconPath">
...
@endif
@if((in_array('assets', user_modules()) &&  user()->permission('view_assets') != 'none') || (in_array('assets', user_modules()) &&in_array('admin', user_roles())))
<x-menu-item icon="question" :text="__('Asset')" :link="route('asset.index')">
                <x-slot name="iconPath">
                <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                <path d="M5.255 5.786a.237.237 0 0 0 .241.247h.825c.138 0 .248-.113.266-.25.09-.656.54-1.134 1.342-1.134.686 0 1.314.343 1.314 1.168 0 .635-.374.927-.965 1.371-.673.489-1.206 1.06-1.168 1.987l.003.217a.25.25 0 0 0 .25.246h.811a.25.25 0 0 0 .25-.25v-.105c0-.718.273-.927 1.01-1.486.609-.463 1.244-.977 1.244-2.056 0-1.511-1.276-2.241-2.673-2.241-1.267 0-2.655.59-2.75 2.286zm1.557 5.763c0 .533.425.927 1.01.927.609 0 1.028-.394 1.028-.927 0-.552-.42-.94-1.029-.94-.584 0-1.009.388-1.009.94z"/>
                </x-slot>
...
@endif
@if((in_array('reimbursement', user_modules()) && user()->permission('view_reimbursement') != 'none') || (in_array('reimbursement', user_modules()) &&in_array('admin', user_roles())))
<x-menu-item icon="question" :text="__('Reimbursement')" :link="route('reimbursement.index')">
                <x-slot name="iconPath">
                <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                <path d="M5.255 5.786a.237.237 0 0 0 .241.247h.825c.138 0 .248-.113.266-.25.09-.656.54-1.134 1.342-1.134.686 0 1.314.343 1.314 1.168 0 .635-.374.927-.965 1.371-.673.489-1.206 1.06-1.168 1.987l.003.217a.25.25 0 0 0 .25.246h.811a.25.25 0 0 0 .25-.25v-.105c0-.718.273-.927 1.01-1.486.609-.463 1.244-.977 1.244-2.056 0-1.511-1.276-2.241-2.673-2.241-1.267 0-2.655.59-2.75 2.286zm1.557 5.763c0 .533.425.927 1.01.927.609 0 1.028-.394 1.028-.927 0-.552-.42-.94-1.029-.94-.584 0-1.009.388-1.009.94z"/>
                </x-slot>
