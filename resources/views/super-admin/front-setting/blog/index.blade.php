        ...
        <x-slot name="buttons">
                <div class="row">
                    ...
                    <div class="col-md-12 mb-2">
                        <x-forms.button-primary icon="plus" class="image-btn mb-2 addBlog" data-type="image">
                            @lang('Add')
...
$('body').on('click', '.addBlog', function () {
        ...
        var blogId = settingId == undefined ? '' : settingId;
        var type = $(this).data('type');
        var url = "{{ route('superadmin.front-settings.blog.create') }}?blogId="+blogId + "&type="+type;
    ...
    $('body').on('click', '.edit-blog', function () {
        var id = $(this).data('id');
        var type = $(this).data('type');
        var url = "{{ route('superadmin.front-settings.blog.edit', ':id')}}?type="+type;
    ...
    $('body').on('click', '.read-more-table-row', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var url = "{{ route('superadmin.front-settings.blog.delete', ':id') }}";
                url = url.replace(':id', id);
