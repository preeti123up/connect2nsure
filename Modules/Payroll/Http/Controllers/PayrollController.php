...
use App\Exports\PayrollExportDetails;
...
class PayrollController extends AccountBaseController
{
    ...
    public function generatePaySlip(Request $request)
    {
        ...
        foreach ($users as $user) {
            ...
            if ($endDate->greaterThan($joiningDate)) {
                ...
                $payDays = $this->countAttendace($startDate, $endDate, $userId, $daysInMonth, $useAttendance, $markAbsentUnpaid, $markApprovedLeavesPaid,$joiningDate,$monthlySalary);
                ...
                if ($payrollSetting->tds_status) {
                    ...
                    if ($payrollSetting->tds_salary < $annualSalary) {
                        ...
                        $taxableSalary = $annualSalary-50000; 
                        ...
                        ->where('month','>=',$financialyearStart->format("n"))
                        ->where('year','=',$financialyearStart->format("Y"))
                        ->sum('tds');
                        if(!is_null($userSlip)){
                ...
                if ($includeExpenseClaims) {
                    $expenseTotal = Expense::where(DB::raw('DATE(purchase_date)'), '>=', $startDate)
                        ...
                        ->where('can_claim', 1)
                        ->sum('price');
                    $reimbursementTotal = Reimbursement::where(DB::raw('DATE(date_of_expense)'), '>=', $startDate)
                        ->where(DB::raw('DATE(date_of_expense)'), '<=', $endDate)
                        ->where('user_id', $userId)
                        ->where('status', 'Approved')
                        ->sum('amount');
                    $expenseTotal =$expenseTotal + $reimbursementTotal;
    ...
    public function countAttendace($startDate, $endDate, $userId, $daysInMonth, $useAttendance, $markAbsentUnpaid, $markApprovedLeavesPaid,$joiningDate,$monthlySalary)
    {
        ...
        if ($useAttendance) {
            ...
            $holidayData = Holiday::getGazettedHolidayByDates($startDate->toDateString(), $endDate->toDateString())->pluck('holiday_date')->values(); // Getting Holiday Data
            
            $holidays = $holidayData->count();

           
            $datesInRange = $this->getDatesInRange($startDate->toDateString(), $endDate->toDateString());
$week_off = 0;
...
foreach ($datesInRange as $date) {
    ...
    // Check if the day of the week is not in the office_open_days array
    if (!in_array($day_of_week, $attendanceSettings->office_open_days)) {
        if ($carbonDate->greaterThan($joiningDate)) {
            $sandwichLeave = Leave::where('leave_date', $carbonDate->toDateString())
                ->where('user_id', $userId)
                ->first();
                if($markAbsentUnpaid){
                    if (!$sandwichLeave) {
            ...
            $totalWorkingDays = $daysInMonth - $holidays - $week_off;
            ...
            $leaveCount = Leave::join('leave_types', 'leave_types.id', 'leaves.leave_type_id')->where('user_id', $userId)
                ...
                ->where('leave_types.type_name', '!=', "Short Leave")
                ->where('status', 'approved')->get();

            $unpaidHaldDayCount = $leaveCount->filter(function ($value, $key) {
                return $value->duration == "half_day" && $value->paid == 0;
            })->count();
            ...
            $PaidLeaveHalfDayCount = $leaveCount->filter(function ($value, $key) {
                return $value->duration == 'half_day' && $value->paid == 1;
            })->count();
            ...
            // $absentCount = ($totalWorkingDays - $presentCount) - ($UnpaidleaveCount + $PaidLeaveCount);
              $absentCount = $UnpaidleaveCount -  ($totalWorkingDays - ($presentCount+$PaidLeaveCount));
            ...
            if ($markAbsentUnpaid) {
              ...
              if($monthlySalary['initialSalary'] > 50000){
                $extraWorkingDays =  $this->countExtraPresentByUser($startDate, $endDate, $userId);
                    $week_off = $week_off - $extraWorkingDays;
            }
            $payDays = $presentCount + $holidays + $week_off;
    ...
    function getDatesInRange($startDate, $endDate) {
        ...
        // Convert start date and end date strings to Carbon instances
        $currentDate = Carbon::parse($startDate);
        $endDate = Carbon::parse($endDate);
    
        // Loop through the dates until the current date is less than or equal to the end date
        while ($currentDate->lte($endDate)) {
            $dates[] = $currentDate->toDateString();
            $currentDate->addDay(); // Increment current date by 1 day
        }
    ...
    public function componentCalculation($components, $basicSalary, $componentValueAmount, $payableSalary, $totalBasicSalary, $earningsTotal, $deductionsTotal, $earnings, $deductions, $salaryId=null)
    {
        ...
        if ($components->component->component_type == 'earning') {
            ...
            elseif ($components->component->value_type == 'basic_percent') {
                $componentValue = ($componentValueAmount / 100) * $totalBasicSalary;
                if($components->component->max_value !== 0 && $componentValue > $components->component->max_value){
                    $deductions[$components->component->component_name] = $components->component->max_value;
                }else{
                    $deductions[$components->component->component_name] = round(floatval($componentValue), 2);  
                }
    ...
    public function calculateTds($taxableSalary, $salaryTds, $previousLimit, $annualSalary, $salaryTdsTotal)
    {
        ...
        foreach ($salaryTds as $tds) {
            ...
            if($tds->salary_to <= $taxableSalary){
                $amount=$amount+(($tds->salary_to-$tds->salary_from)*($tds->salary_percent / 100));
            }elseif($taxableSalary>=$tds->salary_from ) {
                
                $amount=$amount+(($taxableSalary-$tds->salary_from)*($tds->salary_percent / 100));
    ...
    public function countHalfDaysPresentByUser($startDate, $endDate, $userId, $holidayData)
    {
     ...
     public function exportPayroll($start_date, $end_date){
        $users = User::withoutGlobalScope(ActiveScope::class)
           ...
           ->select('users.id', 'users.name', 'users.email', 'users.image', 'designations.name as designation_name', 
           'salary_slips.net_salary','salary_slips.expense_claims','salary_slips.monthly_salary', 'salary_slips.paid_on', 
           'salary_slips.status as salary_status', 'salary_slips.id as salary_slip_id', 
           'salary_slips.added_by', 'salary_slips.month', 'salary_slips.year', 'salary_slips.currency_id','salary_slips.tds','salary_slips.pay_days','salary_slips.monthly_salary')            
           ->where('roles.name', '<>', 'client')
            ->whereDate('salary_slips.salary_from', '=', $start_date)
            ->whereDate('salary_slips.salary_to', '=', $end_date)
        ...
        $daysInMonth = CarbonPeriod::create($start_date->format('Y-m-d'), '1 day', $end_date)->count();
        ...
        foreach ($users as $i => $v) {
            ...
            $data['users'][$i] = [
                ...
                'per_day_salery' => $v->monthly_salary/30,
                'expense_claims' => $v->expense_claims,
                'paid_on' => $v->paid_on,
                'salary_status' => $v->salary_status,
                'salary_slip_id' => $v->salary_slip_id,
                'added_by' => $v->added_by,
                'month' => $v->month,
                'pay_days' => $v->pay_days,
                'year' => $v->year,
            ...
            // Get holiday data
            $holidayData = Holiday::getHolidayByDates($start_date->toDateString(), $end_date->toDateString())
            ->pluck('holiday_date')->values(); 
            $holidays = $holidayData->count();
    
            // Get dates in range
            $datesInRange =  $this->getDatesInRange($start_date->toDateString(), $end_date->toDateString());
    
            // Initialize variables for calculating week off
            $week_off = 0;
            foreach ($datesInRange as $date) {
                $carbonDate = Carbon::parse($date);
                $day_of_week = $carbonDate->dayOfWeek;
            ...
            $fullDayPresentCount = $this->countFullDaysPresentByUser($start_date, $end_date, $v->id, $holidayData); // Getting Attendance Data
            $halfDayPresentCount = $this->countHalfDaysPresentByUser($start_date,$end_date, $v->id, $holidayData); // Getting Attendance Data
            $extraWorkingDays =  $this->countExtraPresentByUser($start_date, $end_date, $v->id);
            $presentCount = $fullDayPresentCount + $halfDayPresentCount;


            $data['users'][$i]['daysInMonth'] = $daysInMonth;
            $data['users'][$i]['totalWorkingDays'] = $totalWorkingDays;
            $data['users'][$i]['week_off'] = $week_off;
            $data['users'][$i]['holidays'] = $holidays;
            $data['users'][$i]['presentCount'] = $presentCount;
            
            // Initialize leave count
            $leaveCount = 0;
            
            // Get leave types count
            $leavetypes = Leave::leftJoin('leave_types', 'leave_types.id', 'leaves.leave_type_id')
                ->where('user_id', $v->id)
                ->whereDate('leave_date', '>=', $start_date)
                ->whereDate('leave_date', '<=', $end_date)
                ->where('status', 'approved')
                ->where('leave_types.type_name','<>' ,'Short Leave')
                ->groupBy('leave_types.id')
                ->select('leave_types.type_name', DB::raw('SUM(CASE WHEN duration = "half_day" THEN 0.5 ELSE 1 END) as count'))
                ->get();
            $allLeaveTypes = LeaveType::pluck('type_name')->toArray();
            $leaveTypesData = [];
            foreach ($leavetypes as $leave) {
                $leaveTypesData[$leave->type_name] = $leave->count;
                $leaveCount=$leaveCount+$leave->count;
            }
    
            // Fill in leave types with zero count
            foreach ($allLeaveTypes as $type) {
                if (!isset($leaveTypesData[$type])) {
                    $leaveTypesData[$type] = 0;
                }
            }
             ksort($leaveTypesData);
            // Update user data with leave types count and absent count
            $data['users'][$i]['extraWorkingDays'] = $extraWorkingDays;
            $data['users'][$i]['extra_salary'] = $data['users'][$i]['per_day_salery'] * $data['users'][$i]['extraWorkingDays'];
            $data['users'][$i]['leaveTypesCount'] = $leaveTypesData;
            $data['users'][$i]['absentCount'] = $presentCount+$leaveCount < $totalWorkingDays?$totalWorkingDays-($presentCount+$leaveCount):0 ;
        }
     
        return Excel::download(new PayrollExport($data), $start_date . '.xlsx');
    }
    
   
    public function exportPayrollDetails($start_date, $end_date){
        ...
        foreach ($users as $i => $v) {
            ...
            $data['users'][$i] = [
                ...
                'company_acount_no'=>user()->company->account_no,
                'company_bank_name'=>user()->company->bank_name,
            ];
    
        }
        return Excel::download(new PayrollExportDetails($data), $start_date . '.xlsx');
      }
      
      
       public function countExtraPresentByUser($startDate, $endDate, $userId)
    {
        // Get dates in range
        $datesInRange = $this->getDatesInRange($startDate->toDateString(), $endDate->toDateString());
        $extra_working = 0;
        ...
        foreach ($datesInRange as $date) {
            ...
            // Get shift schedule for the user on the specific date
            $attendanceSettings = EmployeeShiftSchedule::with('shift')
                ->where('user_id', $userId)
                ->whereDate('date', $date)
                ->first();
            
            // If no specific shift, use the default attendance settings
            if ($attendanceSettings) {
                $attendanceSettings = $attendanceSettings->shift;
            } else {
                $attendanceSettings = AttendanceSetting::first()->shift;
            ...
            // Check if the day is not an office open day
            if (!in_array($day_of_week, $attendanceSettings->office_open_days)) {
                $half_day_count = Attendance::select(DB::raw('count(DISTINCT DATE(attendances.clock_in_time)) as presentCount'))
                    ->where('user_id', $userId)
                    ->where('half_day', 'yes')
                    ->whereDate(DB::raw('DATE(attendances.clock_in_time)'), $carbonDate->toDateString())
                    ->first();
    
                $full_day_count = Attendance::select(DB::raw('count(DISTINCT DATE(attendances.clock_in_time)) as presentCount'))
                    ->where('user_id', $userId)
                    ->where('half_day', 'no')
                    ->whereDate(DB::raw('DATE(attendances.clock_in_time)'), $carbonDate->toDateString())
                    ->first();
    
                $half_day_count = $half_day_count ? $half_day_count->presentCount : 0;
                $full_day_count = $full_day_count ? $full_day_count->presentCount : 0;
    
                $extra_working += $full_day_count + ($half_day_count / 2);
            }
        }
