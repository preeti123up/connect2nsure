...
ApiRoute::group(['namespace' => 'Modules\RestAPI\Http\Controllers'], function () {
    ...
    ApiRoute::post('auth/verify-token', ['as' => 'api.auth.verifyToken', 'uses' => 'AuthController@verifyResetPasswordToken']);
...
ApiRoute::group(['namespace' => 'Modules\RestAPI\Http\Controllers', 'middleware' => ['auth:sanctum', 'api.auth']], function () {
    ...
    ApiRoute::get('auth/logout', ['as' => 'api.auth.logout', 'uses' => 'AuthController@logout']);
    ApiRoute::get('auth/refresh', ['as' => 'api.auth.refresh', 'uses' => 'AuthController@refresh']);
    ApiRoute::post('auth/change-password', ['as' => 'api.auth.change-password', 'uses' => 'AuthController@changePassword']);
    ApiRoute::post('auth/change-fcm', ['as' => 'api.auth.change-fcm', 'uses' => 'AuthController@changeFcm']);
  ...
  // ApiRoute::resource('holiday', 'HolidayController');
    ApiRoute::post('holiday', 'HolidayController@Holiday');
    ...
    ApiRoute::resource('expense', 'ExpenseController');
     ...
     ApiRoute::post('/leave/apply', ['as' => 'leave.apply', 'uses' => 'LeaveController@apply']);
    ApiRoute::get('leave/list', ['as' => 'leave.list', 'uses' => 'LeaveController@LeaveList']);
    ApiRoute::get('leave/types', ['as' => 'leave.types', 'uses' => 'LeaveController@leaveType']);
    ApiRoute::get('team/leave/{status?}', ['as' => 'team.leave', 'uses' => 'LeaveController@TeamLeave']);
    ApiRoute::get('team/members', ['as' => 'team.members', 'uses' => 'LeaveController@teamMembers']);
    ApiRoute::delete('leave/delete', ['uses' => 'LeaveController@delete']);
    ApiRoute::put('leave/{status}', ['uses' => 'LeaveController@updateStatus']);
    ApiRoute::post('leave/leave-file', ['uses' => 'LeaveController@addFile']);
    ...
    ApiRoute::post('/attendance/clock-out',[ 'as' => 'attendance.clockOut',  'uses' => 'AttendanceController@clockOut',]);
    ApiRoute::get('/attendance/team',[ 'as' => 'attendance.team',  'uses' => 'AttendanceController@teamAttendance',]);
    ApiRoute::get('/attendance', 'AttendanceController@attendance');
    ApiRoute::get('/celebration', 'CelebrationController@Celebration');
    ApiRoute::get('/payslip', 'PayrollController@payslip');
    ApiRoute::post('/wish', 'WishController@sendWish');
    ApiRoute::post('/reply', 'WishController@Reply');
    ApiRoute::post('/reimbursment', 'ReimbursementController@Reimbursment');
    ApiRoute::get('/reimbursment', 'ReimbursementController@getReimbursment');
   ApiRoute::delete('/reimbursment/{uniqueId}', 'ReimbursementController@ReimbursmentDelete');
    
     //asset
  ApiRoute::get('/list', 'AssetController@asset');
   ApiRoute::get('return/asset/{id}', 'AssetController@returnAsset');
    
       //Face Attendance api
    ApiRoute::get('fa/employees', 'FaceAttendanceController@employeeList');
    ApiRoute::get('fa/enrolled-employees', 'FaceAttendanceController@enrollEmployeeList');
    ApiRoute::post('fa/register', 'FaceAttendanceController@FaceRegister');
    ApiRoute::post('fa/update', 'FaceAttendanceController@FaceUpdate');
    ApiRoute::post('fa/clock-in-out', 'FaceAttendanceController@FaceclockInOut');
    ApiRoute::post('fa/sync-clock-in-out', 'FaceAttendanceController@SyncClockInOut');
    
    ApiRoute::get('onboard-screen', 'OnboardScreenController@index');
